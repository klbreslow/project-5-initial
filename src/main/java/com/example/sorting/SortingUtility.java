...
public class SortingUtility {
    ...
    public static <T extends Comparable<T>> void gnomeSort(T[] data) {
        ...
        while (pos < data.length) {
            ...
            if (pos == 0 || (Integer.parseInt(data[pos].toString()) >= Integer.parseInt(data[pos - 1].toString()))) {
                pos = pos + 1;
            } else {
                swap(data, pos, pos - 1);
                pos = pos - 1;
    ...
    private static <T extends Comparable<T>> void gnomierSort(T[] data, int upperBound) {
        ...
        while (pos > 0 && (Integer.parseInt(data[pos - 1].toString()) > Integer.parseInt(data[pos].toString()))){
            swap(data, pos, pos - 1);
            pos = pos - 1;
        }
    }
